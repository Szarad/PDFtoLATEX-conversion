#!/usr/bin/env python
"""
Implemented from main, example project: https://github.com/harvardnlp/im2markup
"""
...
def main(args):
    ...
    cmd = "cat %s | node prepare_input/preprocess_latex.js %s > %s "%(temp_file, parameters.mode, output_file)
    ret = subprocess.call(cmd, shell=True)
    os.remove(temp_file)
    if ret != 0:
        logging.error('FAILED: %s'%cmd)
